...
fn marker_beam_search_log(
    ...
    marker_interval: usize,
    marker_sequence_str: String,
) -> PyResult<(String, Vec<usize>, f32, u64)> { // ) -> Result<(String, Vec<usize>, f32, u64)
    // Convert alphabet
    let alphabet = seq_to_vec(alphabet)?;
    ...
    unsafe {
        let network_output = network_output.as_array();
        py.allow_threads(|| {
            code_aware_beam_search::marker_beam_search_log(
                &network_output,
                &alphabet,
                beam_size,
                beam_cut_threshold,
...
// fn for beam_search_log_with_base_probabilities
#[cfg(feature = "python")]
#[pyfunction(
    beam_size = "5",
    beam_cut_threshold = "0.0",
...
#[pyo3(
    text_signature = "(network_output, alphabet, beam_size=5, beam_cut_threshold=0.0, collapse_repeats=True, forward_primer, reverse_primer, offset_sequence)"
)]
fn beam_search_log_with_base_probabilities(
    py: Python,
    network_output: &PyArray2<f32>,
    alphabet: &PySequence,
    beam_size: usize,
    ...
    unsafe {
        let network_output = network_output.as_array();
        py.allow_threads(|| {
            code_aware_beam_search::beam_search_log_with_base_probabilities(
                &network_output,
                &alphabet,
                beam_size,
                beam_cut_threshold,
